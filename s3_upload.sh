...
if [ $leng -lt $loc_leng ] #if directories in S3 is less than local one
then
        ...
        do
                ...
                aws s3 cp $i/$date_now.log s3://refine-logs.s3.mng.refinehub.com/syslog/$device/

                touch /var/log/logstash/s3_upload_log/s3_upload_$date_now.success
        done
else
